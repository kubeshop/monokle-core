...
type FilterBarProps = {
  ...
  setFilters: (filters: {namespaces: string[]; kinds: string[]; maxDepth: number}) => void;
  defaultNamespace?: string;
};

export function FilterBar({graphRef, resources, resourceMap, setFilters, defaultNamespace}: FilterBarProps) {
  const filterBarRef = useRef<HTMLDivElement>(null);
  const [isDownloading, setIsDownloading] = useState(false);

  const [kind, setKind] = useState();
  const [namespace, setNamespace] = useState<string>(ALL_NAMESPACES.value);
  ...
  useEffect(() => {
    if (!defaultNamespace || !namespaceList.find(n => n.value === defaultNamespace)) {
      return;
    }

    setNamespace(defaultNamespace);
  }, [defaultNamespace, namespaceList]);

  return (
    <Panel position="top-center" style={{width: 'calc(100% - 32px)', margin: '15px 0'}}>
      <Bar ref={filterBarRef}>
        <Space size="middle" style={{flexWrap: 'wrap', rowGap: 0}}>
          ...
          />
          <Filter label="Show" menuItems={[ALL_KINDS, ...kindList]} defaultValue={ALL_KINDS.value} onChange={setKind} />
          <Filter label="View" menuItems={VIEW_DEPTHS} defaultValue={maxDepth} onChange={setMaxDepth} />
        </Space>
        {isDownloading ? (
...
function Filter({label, menuItems, onChange, defaultValue, value}: FilterProps) {
  return (
    <Space size={0}>
