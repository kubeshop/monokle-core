import { ITreeNavigator, SectionBlueprint, SectionInstance } from "../types";
...
export function computeSectionVisibility(
  ...
  sectionBlueprint.childSectionIds?.forEach((childSectionId, index) => {
    ...
    const childSectionBlueprint = treeNavigator.getSectionBlueprint(childSectionInstance.id);
