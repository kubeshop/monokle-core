...
export class ResourceLinksValidator extends AbstractPlugin {
  ...
  override merge(_previous: ValidationResult[], current: ValidationResult[]): ValidationResult[] {
  ...
  private async validateResource(resource: Resource): Promise<ValidationResult[]> {
    ...
    const unsatisfiedRefs = refs.filter(
      ref => ref.type === ResourceRefType.Unsatisfied || ref.type === ResourceRefType.UnsatisfiedOwner
    );

    return unsatisfiedRefs.map(ref => this.adaptToValidationResult(resource, ref)).filter(isDefined);
  }
  ...
  private adaptToValidationResult(resource: Resource, ref: ResourceRef) {
    ...
    if (ref.target?.type === 'resource' && ref.target?.isOptional) {
      return this.isRuleEnabled('LNK002')
        ? this.createValidationResult('LNK002', {
            message: {
      ...
      return this.isRuleEnabled('LNK003')
        ? this.createValidationResult('LNK003', {
            message: {
      ...
      return this.isRuleEnabled('LNK001')
        ? this.createValidationResult('LNK001', {
            message: {
